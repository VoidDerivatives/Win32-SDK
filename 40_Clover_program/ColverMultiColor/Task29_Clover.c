						...
						LPARAM lParam)
{
	...
	case WM_PAINT:
			...
			for(dAngle = 0.0; dAngle < TWO_PIE;dAngle = dAngle + TWO_PIE/360)
			{
				...
				if((dAngle >= (TWO_PIE / 4))  && (dAngle <= (TWO_PIE /2)))
				{
					
					hPen = CreatePen(PS_SOLID , 1 , RGB(255,0,0));   // Red
				}
				else if(dAngle >= (TWO_PIE /2) && (dAngle <= (3 * (TWO_PIE)) / 4))
				{
					
					hPen = CreatePen(PS_SOLID , 1 , RGB(163,73,164));  // Purple
				}
				else if((dAngle >= (3*(TWO_PIE)) / 4)  && (dAngle <= TWO_PIE))
				{
					
					hPen = CreatePen(PS_SOLID , 1 , RGB(0,255,0));    // Green
				}
				else
				{
					
					hPen = CreatePen(PS_SOLID , 1 , RGB(0,0,255));    // Blue
				}
