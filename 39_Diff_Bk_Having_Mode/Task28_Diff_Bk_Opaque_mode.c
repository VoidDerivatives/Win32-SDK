					...
					)
{
	...
	hwnd = CreateWindow(
					szAppName,
					TEXT("DIFFERENT BK COLOR WITH OPEQUE MODE"),
					WS_OVERLAPPEDWINDOW,
					x,
					y,
					WINDOW_WIDTH,
						...
						LPARAM lParam)
{
	...
	case WM_PAINT:
			...
			hPen = CreatePen(PS_DASHDOTDOT , 1 , RGB(1,71,68));
			SelectObject(hdc,hPen);	

			hBrush = CreateHatchBrush(HS_CROSS,RGB(221,30,2));
			SelectObject(hdc,hBrush);
			...
			SelectObject(hdc,hPen);
			hBrush = CreateHatchBrush(HS_BDIAGONAL,RGB(120,5,28));
			SelectObject(hdc,hBrush);	
			...
			DrawText(hdc,TEXT("No Matter where you from , Your Dreams are valid..."),-1,
