...
void logger(char *text,int ivalue,char a)
{
						...
						LPARAM lParam)
{
	...
	int iVirtKeyCtrl;				// Virtual - key code
	HRESULT hResult; 
	...
	static int iCaretPosXTemp1;
	static int iCaretPosYTemp1;
	POINT point;
	...
	case WM_CHAR:
		...
		switch(wParam)
		{
			case VK_BACK:
					...
					logger("iCurChar", iCurChar, 'a');

						if(iCaretPosX == 0 && iCaretPosY == 0)
						{
							MessageBeep((UINT) - 1);   		// Simple beep
						}
						else if(cch <= iCurChar)
						{
							if(pchInputBuf[iCurChar] == VK_RETURN || pchInputBuf[iCurChar] == 0x0A)
							{
								iCurChar--;
								
							}
							
							logger("iCurChar", iCurChar, pchInputBuf[iCurChar]);
							if(iCaretPosX == 0)    //  only when 
							{
								logger("iCaretPosX", iCaretPosX, 'X');
							...
							if(pchInputBuf[cch-1] == 0x0A || pchInputBuf[cch-1] == VK_RETURN)
							{
							}
							else
							{
								hdc = GetDC(hwnd);
								ch = pchInputBuf[cch-1];
								pchInputBuf[cch-1] = '\0';
								logger("ch \0", i, ch);
	...
	case WM_KEYDOWN:
			...
			switch(wParam)
			{
				...
				case VK_LEFT:
						...
						if(pchInputBuf[iCurChar] != VK_RETURN)
						{
							hdc = GetDC(hwnd);
							if(iVirtKey & SHIFTED)
							{
				...
				case VK_DOWN:
						...
						else
						{
								...
								while(pchInputBuf[i] != VK_RETURN &&  i <= cch )
								{
									...
									if(iVirtKey & SHIFTED)   // When shift is press then select upto end of that line
									{
										hdc = GetDC(hwnd);
										crPrevText = SetTextColor(hdc,RGB(255,255,255));
										crPrevBk = SetBkColor(hdc,RGB(0,0,0));
								...
								while(iCaretPosXTemp - 2 > iCaretPosX)
								{
									...
									if(iVirtKey & SHIFTED)    // select start from the start upto X pos limit
									{
										hdc = GetDC(hwnd);
										crPrevText = SetTextColor(hdc,RGB(255,255,255));
										crPrevBk = SetBkColor(hdc,RGB(0,0,0));
				...
				case VK_HOME :
						...
						iCaretPosXTemp1 = iCaretPosX;
						iCaretPosYTemp1 = iCaretPosY;
						iVirtKey = GetKeyState(VK_SHIFT);
						iVirtKeyCtrl = GetKeyState(VK_CONTROL);
						iCurCharTemp = iCurChar;  // for ctrl and shift press together
						for(i = iCurChar ; ; i--)
						{
							
							if((iVirtKeyCtrl & SHIFTED) && (iVirtKey & SHIFTED))
							{
								if(i == 0)
									break;
								// when ctrl and shift both press together
							}
							else
							{
								if(i != 0 && pchInputBuf[i - 1] != VK_RETURN && pchInputBuf[i - 1] != 0x0A )
								{}
								else 
									break;
							}
						...
						if((iVirtKeyCtrl & SHIFTED) && (iVirtKey & SHIFTED))
						{
							iCurChar = iCurCharTemp;
							iCaretPosX = iCaretPosXTemp1;
							iCaretPosY = iCaretPosYTemp1;
							
						}
						else if(iVirtKey & SHIFTED)
						{
							iCaretPosX = iCaretPosXTemp;
						}
						else if(iVirtKeyCtrl & SHIFTED)
						{
							iCaretPosX = 0;
							iCaretPosY = 0;
							iCurChar = 0;
				...
				case VK_END:
						...
						iVirtKeyCtrl = GetKeyState(VK_CONTROL);
						iCaretPosXTemp = iCaretPosX;
						iCaretPosYTemp = iCaretPosY;
						iCurCharTemp = iCurChar;
						for(i = iCurChar; ;i++)
						{
							if(iVirtKeyCtrl & SHIFTED)
							{
							...
							{
								if(i < cch && pchInputBuf[i] != VK_RETURN && pchInputBuf[i] != 0x0A)
								{}
								else 
									break;
							}
							...
							if(iVirtKey & SHIFTED || ((iVirtKey & SHIFTED) && (iVirtKeyCtrl & SHIFTED)))    
							{
								crPrevText = SetTextColor(hdc,RGB(255,255,255));
								crPrevBk = SetBkColor(hdc,RGB(0,0,0));
								if(i != cch)					
                        ...
                        if((iVirtKey & SHIFTED) && (iVirtKeyCtrl & SHIFTED))
						{
							iCaretPosX = iCaretPosXTemp;
							iCaretPosY = iCaretPosYTemp;
							iCurChar = iCurCharTemp;
						}
						else
						{
